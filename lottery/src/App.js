...
const App = () =>
{
  ...
  function toggleColor(id)
  {
    ...
    else
    {
      let updatedCounter = counter - 1
      setCounter(updatedCounter);
      document.getElementById(id).classList.remove('button2')
  ...
  function resetGame()
  {
    lotteryPool.map((ball) =>
    {
      ...
      {
        document.getElementById(ball.value).classList.remove('button2')
        document.getElementById(ball.value).classList.add('button')
      }
    });
    setlotteryPool([]);
    setCounter(0);
  ...
  return (
    <div className="App">
      ...
      <div class="button-container">
        <button class="button-28" role="button" key="bu-on" onClick={() => resetGame()}>Reset</button>
        <button class="button-28" role="button" key="bu-on1" onClick={() => startLottery()}>Play</button>
        <button class="button-28" role="button" key="bu-on2" onClick={() => pickLuckyDip()}>Lucky Dip</button></div>
      ...
      <div id="popup_box">
        <input type="button" id="cancel_button" value="X" onClick={() => {hidepopup(); resetGame()}}></input>
        <p id="info_text">To be populated.
        </p><input type="button" id="close_button" value="Close" onClick={() => { hidepopup(); resetGame() }}></input>
      </div>
    </div>
  );
};
